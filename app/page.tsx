...
const Home = () => {
          ...
          return (
            <div key={user.id} className="border border-red-700 p-4">
              <div className="mb-4">
                ...
                <div className="mb-1">
                  Website:{" "}
                  <Link href={user.website}>{user.website.length}</Link>
                </div>
              </div>
              <div className="mb-4">
                ...
                <div className="mb-1">Street: {user.address.street}</div>
                <div className="mb-1">Suite: {user.address.suite}</div>
                <div className="mb-1">City: {user.address.city}</div>
                <div className="mb-1">ZIP: {user.address.zipcode}</div>
                <div className="ml-4">
              ...
              <div className="mb-4">
                ...
                <div className="mb-1">Company Name: {user.company.name}</div>
                <div className="mb-1">
                  Company Catch Phrase:
                  {user.company.catchPhrase}
                </div>
                <div className="mb-1">Company BS: {user.company.bs}</div>
              </div>
            </div>
          );
        })}
  ...
  if (!isLoading && !isError) {
    return (
      <>
        <Link href={"/CalenderView"}>Cal</Link>
        {content}
      </>
    );
  } else {
